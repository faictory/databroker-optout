...
class TestE2EMakeRun:
    ...
    def test_make_run_creates_tracker_json(self):
        ...
        for broker_slug, broker_info in brokers.items():
            ...
            assert 'exposure_ids' in broker_info, f"Broker entry {broker_slug} should have 'exposure_ids' field"
            ...
            # Verify exposure_ids is a list with at least one entry
            assert isinstance(broker_info['exposure_ids'], list), \
                f"Broker {broker_slug} exposure_ids should be a list"
            assert len(broker_info['exposure_ids']) > 0, \
                f"Broker {broker_slug} exposure_ids should have at least one entry"
